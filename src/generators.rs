...
///
/// ```no_run
/// use gitkyl::generate_markdown_blob_page;
/// use std::path::Path;
///
/// let html = generate_markdown_blob_page(
///     Path::new("."),
///     "main",
///     Path::new("README.md"),
///     "my-repo"
...
/// # Ok::<(), anyhow::Error>(())
/// ```
pub fn generate_markdown_blob_page(
    repo_path: impl AsRef<Path>,
    ref_name: &str,
    file_path: impl AsRef<Path>,
    repo_name: &str,
...
mod tests {
    ...
    #[test]
    fn test_generate_markdown_blob_page() {
        // Arrange
        let repo_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        ...
        // Assert
        assert!(result.is_ok(), "Should generate markdown blob page");
        let html = result.unwrap().into_string();
        assert!(
            html.contains("markdown-content"),
            "Should have markdown content class"
        );
        assert!(html.contains("<!DOCTYPE html>"), "Should be valid HTML");
        assert!(html.contains("Gitkyl"), "Should contain repository name");
    }

    #[test]
    fn test_generate_markdown_blob_page_renders_gfm() {
        // Arrange
        let repo_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
